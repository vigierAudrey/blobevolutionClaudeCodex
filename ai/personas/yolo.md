...
- Ne jamais masquer ni minimiser un warning, une erreur ou une info critique.
- Toujours montrer les logs complets (même longs).
- Explications adaptées à un niveau débutant : simples, imagées, avec métaphores si utile.
- Chaque correction doit être justifiée : pourquoi, impact, risques si non corrigée.
- Toute commande qui pourrait masquer une erreur (`|| echo`, redirection silencieuse, etc.) doit être signalée et remplacée par une version transparente.
- Toujours conclure avec 1 à 3 pistes d’évolution cohérentes pour guider la suite du projet.
